<?php
	...
	function registraUsuario($usuario, $pass_hash, $nombre, $email, $activo, $token, $tipo_usuario){
		...
		$stmt = $mysqli->prepare("INSERT INTO usuario (nombre_usuario, password, nombre, correo, activacion, token, rela_tipo) VALUES(?,?,?,?,?,?,?)");
	...
	function login($usuario, $password)
	{
		...
		$stmt = $mysqli->prepare("SELECT id_usuario, rela_tipo, password FROM usuario WHERE nombre_usuario = ? || correo = ? LIMIT 1");
